...
#             self.vocoder2 = vocoder
    ...
    def __call__(
        ...
        if self.vocoder is not None:
            ...
            if (input_feat.shape[0]>100):
              idx = torch.arange(0,100)
              input_feat = input_feat[idx, :]

            else:
              for i in range(100 - input_feat.shape[0]):
                x = torch.zeros(1, 80)
                input_feat = torch.cat((input_feat, x), 0)
...
#             self.vocoder2 = vocoder
    ...
    def __call__(
        ...
        if self.vocoder is not None:
            ...
            # CODE FOR CHANGE IN "VOLUME"
##            input_feat = 2*input_feat
##            if (input_feat.shape[0]>100):
##              idx = torch.arange(0,100)
##              input_feat = input_feat[idx, :]
##
...
##                input_feat = torch.cat((input_feat, x), 0)
            ...
            # CODE FOR FUZZ AT THE END
            if (input_feat.shape[0]>80):
              idx = torch.arange(0,80)
              input_feat = input_feat[idx, :]
              for i in range(20):
                x = torch.zeros(1, 80)
                input_feat = torch.cat((input_feat, x), 0)
...
# CODE FOR FUZZ EVERYWHERE
#            input_feat = torch.ones(100,80)
